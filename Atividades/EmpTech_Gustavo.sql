create database if not exists EmpTech_Gustavo;
use EmpTech_Gustavo;
...
/*b) Crie um procedimento armazenado chamado "AddVenda" que insere uma nova atuação de vendas na tabela 'AtuaçãoVendas'. O procedimento deve receber uma descrição e adicionar a nova atuação de vendas*/
delimiter //
...
BEGIN
    DECLARE msg TEXT;
    -- Verifica se já existe um registro com o codIndicado informado
    IF EXISTS (
        SELECT 1 
        FROM indicacoes 
        WHERE codIndicado = NEW.codIndicado
        ...
        SET msg = 'O funcionário indicado já foi indicado por outro funcionário.';
        -- Lança o erro
